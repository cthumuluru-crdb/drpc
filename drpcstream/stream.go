...
// HandleFrame accepts a frame, assembles it into a packet, and once the packet
// is complete (fr.Done is true), calls HandlePacket and resets the in-progress
// packet state.
func (s *Stream) HandleFrame(fr drpcwire.Frame) (err error) {
	if fr.ID.Stream != s.id.Stream {
		return nil
	}
